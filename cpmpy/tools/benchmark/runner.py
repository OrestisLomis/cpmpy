...
csv.field_size_limit(10000*1024*1024)
...
def execute_instance(args: Tuple[callable, str, dict, callable, str, int, int, int, str, bool, bool, str]) -> None:
    ...
    # fieldnames = list(metadata.keys()) + ['solver', 'time_total', 'time_parse', 'mus_size', 'solution', 'status']
    ...
    try:
        ...
        if os.path.exists(lock_file):
            try:
                ...
                print(f"Warning: Could not remove lock file {lock_file}.", flush=True)
